...
@function_tool
def getMenu():
    return ["Ice Cream", "Zinger Burger", "Cheese Pizza"]

agent = Agent(
    name="Mcdonald Menu Agent",
    instructions="""
...
card = AgentCard(
    ...
    version="1.0.0",
    skills=[AgentSkill(
        name="GetMcdonaldMenu",
        description="Get menu of the MCDonalds",
        id="123",
        tags=["menu", "food", "mcdonald"]
    )],
...
if __name__ == "__main__":
    ...
    import uvicorn
    uvicorn.run(start.build(), host="localhost", port=8001)