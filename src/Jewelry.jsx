    ...
    <div className="bg-black min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <p className="col-span-full text-center text-amber-100/60">No jewelry found</p>
        ) : (
          filteredProducts.map((product) => (
            ...
            >
              {/* Trending Badge */}
              {product.is_trending && (
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute top-3 left-3 z-20 px-2.5 py-1 bg-gradient-to-r from-amber-500 to-yellow-600 rounded-full flex items-center gap-1 shadow-lg">
              ...
              <div className="p-4 space-y-2">
                ...
                <p className="text-xs text-amber-100/60 line-clamp-2">{product.description.slice(0, 50)}{product.description.length > 50 && "..."}</p>
                {product.rating && (
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className={`w-3 h-3 ${i < product.rating ? "fill-amber-400 text-amber-400" : "text-amber-400/20"}`} />
                    ))}
                  </div>
                )}
                <p className="text-xl font-bold bg-gradient-to-r from-amber-400 to-yellow-600 bg-clip-text text-transparent">PKR {product.price}</p>
                <motion.button
